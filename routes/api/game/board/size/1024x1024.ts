...
// Bitmask representing the game board, where each bit corresponds to a cell in the 1024x1024 grid (1024 * 1024 cells total)
let bitmaskBoard: number = 0;

// Array representing player ownership for each cell on the board (1024 * 1024 cells), initialized to null (no ownership)
let playerBoard: (string | null)[] = Array(1024 * 1024).fill(null);
...
function startNewGame() {
  ...
  playerBoard = Array(1024 * 1024).fill(null);       // Clear the player ownership board
...
function endGame() {
  ...
  if (!gameState.winner) {
    const winningIndex = Math.floor(
      crypto.getRandomValues(new Uint32Array(1))[0] / (2 ** 32 - 1) * (1024 * 1024),  // Randomly pick a winning cell index
    );
