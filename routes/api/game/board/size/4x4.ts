...
// Bitmask representing the game board, where each bit corresponds to a cell in the 4x4 grid (16 cells total)
let bitmaskBoard: number = 0;

// Array representing player ownership for each cell on the board (16 cells), initialized to null (no ownership)
...
function startNewGame() {
  ...
  playerBoard = Array(16).fill(null);       // Clear the player ownership board
...
function endGame() {
  ...
  if (!gameState.winner) {
    const winningIndex = Math.floor(
      crypto.getRandomValues(new Uint32Array(1))[0] / (2 ** 32 - 1) * 16,  // Randomly pick a winning cell index
    );
